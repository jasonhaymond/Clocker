...
| `ANDROID_GOOGLE_MAPS_API_KEY` | Needed only for the job-location "Choose on Map" picker's Android build (`react-native-maps` — iOS uses Apple Maps for free, no key needed). Not `EXPO_PUBLIC_`-prefixed: read by `app/app.config.js` at build/config time, not inlined into the JS bundle. Run `npm run configure` to set it interactively, or get one yourself from the [Google Cloud Console](https://console.cloud.google.com/google/maps-apis) (enable "Maps SDK for Android", restrict it to `com.haymondtechnologies.clocker` + the SHA-1 fingerprint `eas credentials` prints for your signing setup) and set it by hand. Without it, "Choose on Map" just shows an "unavailable" message — "Use My Current Location" and "Enter an Address" both still work regardless, since neither touches `react-native-maps`. |
