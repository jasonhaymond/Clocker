...
- **Backups, both clients: in-app instructions for a dedicated remote backup user.** A
  ...
  selectable`, tap-and-hold) that create a restricted system account (`nologin` shell),
  the repository directory, and an `authorized_keys` entry locked to `borg serve
  --restrict-to-repository ...` — so the key can't do anything beyond running backups
  even if leaked. Generated by a new shared helper, `shared/src/backupRemoteSetup.ts`
  (parses the already-typed "Repo URL" for a `user@host:path` to prefill the script's
  host/path, falling back to the convention `docs/deployment.md` already documented) —
  `docs/deployment.md#the-host-agent` now points at this generator instead of carrying a
  separate hand-written snippet that could drift from it. Verified for real via
