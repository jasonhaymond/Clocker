...
- **Backups, both clients: in-app instructions for a dedicated remote backup user.** A
  ...
  selectable`, tap-and-hold) that create a restricted system account (real `/bin/sh`
  shell — a `nologin` shell was originally used here and broke the backup entirely; see
  the real-bug-fix entry right below for why), the repository directory, and an
...
- **Real bug (2026-09-12, reported by the user on a real first backup attempt): the
  ...
  Fixed in `shared/src/backupRemoteSetup.ts`: the generated script now uses a real
  `/bin/sh` shell — the `restrict`+`command=` entry in `authorized_keys` already fully
  locks the account down regardless of shell, so this doesn't reopen anything security-
  wise. **This does NOT retroactively fix an account already created on a real remote
  server** — the user needs to run `sudo usermod -s /bin/sh clocker-backup` (or their
  actual username) on that server themselves; flagged to them directly, not something
